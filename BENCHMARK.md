...
- **Hand-written surface (smaller = more readable): 48,756 lines** — Our hand-written surface is ~738× smaller than Chromium's cited ~36,000,000 lines — one person can read it front to back. (Honest caveat: smaller surface buys readability, not feature parity.)
- **compat-per-LOC (North Star): 0.0009 passes/line** — compat-per-LOC is our North Star: passing checks per hand-written line. Chromium publishes no per-LOC figure (needs-source), but spreading comparable compatibility over ~36M lines yields orders of magnitude less per line — this axis is structurally ours.
- **mechanism-density (features per 1k hand-written lines): 18.30 features/kloc** — 892 platform features (CSS properties + DOM members) over 48,756 hand-written lines = 18.30 features / 1k lines. Platform-as-Data makes coverage grow per data row, not per hand-written line — the mechanism advantage Chromium's hand-rolled surface cannot match.
...
|---|---|---|---|
| Hand-written surface (smaller = more readable) | 48,756 lines | 36,000,000 lines _(Wikipedia — Chromium (web browser))_ | 🟢 WIN |
| compat-per-LOC (North Star) | 0.0009 passes/line | _needs-source_ | 🟢 WIN |
| mechanism-density (features per 1k hand-written lines) | 18.30 features/kloc | — | 🟢 WIN |
| CSS-property coverage (raw count) | 340 properties | 600 properties _(CSS specifications (order-of-magnitude))_ | 🟡 GAP |
| Raw WPT / Interop pass rate | 100% of maintained WPT subsets (462/462 curated subtests) | 95% _(WebKit blog — The success of Interop 2024)_ | 🟡 GAP |
| Runtime performance (Speedometer-class) | not measured | _needs-source_ | ⚪ N/C |

## Our live metrics (re-computed every run)

- Hand-written product lines: **48,756**
- Generated lines: 5,439
- Test lines: 26,345
- Total system size: 80,540
...
- **compat-per-LOC: 0.0009 passes/line**
- **mechanism-density: 18.30 features/kloc**

## Execution evidence (maintained WPT subset trace)

- Maintained subsets traced: 6
- WPT files/subtests: 110 files, 462 subtests
- WPT outcomes: 462 passed, 0 failed, 0 errored
- Query calls: 11290
- Recomputation events: 3793
- Cache hits: 7497
- Verified cache hits: 23
- Dependency reads: 181295
...
- Ours (live): 48,756 lines
- Chromium (cited): 36,000,000 lines _(Wikipedia — Chromium (web browser))_
- Our hand-written surface is ~738× smaller than Chromium's cited ~36,000,000 lines — one person can read it front to back. (Honest caveat: smaller surface buys readability, not feature parity.)

### compat-per-LOC (North Star) — 🟢 WIN

- Ours (live): 0.0009 passes/line
...
- Ours (live): 18.30 features/kloc
- Chromium (cited): —
- 892 platform features (CSS properties + DOM members) over 48,756 hand-written lines = 18.30 features / 1k lines. Platform-as-Data makes coverage grow per data row, not per hand-written line — the mechanism advantage Chromium's hand-rolled surface cannot match.
...
- Ours (live): 100% of maintained WPT subsets (462/462 curated subtests)
